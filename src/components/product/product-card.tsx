...
export function ProductCard({ product, index }: ProductCardProps) {
  return (
    <article className="overflow-hidden rounded-lg border border-soft-border bg-surface shadow-sm transition hover:-translate-y-1 hover:shadow-md">
      <div className={`relative h-56 bg-gradient-to-br ${product.tint}`}>
        <span className="absolute left-4 top-4 rounded bg-brand-soft px-2 py-1 text-[10px] font-bold text-brand">
          {product.tag}
        </span>
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex items-center gap-1 text-rating">
          ...
          <span className="ml-2 text-xs text-ink-subtle">98</span>
        </div>
      </div>
    </article>
  );
